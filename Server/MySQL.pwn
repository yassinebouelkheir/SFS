...
stock CreateTables()
{
	...
	mysql_tquery(Database, "CREATE TABLE IF NOT EXISTS `CONNECTIONS` (`ID` INT(16) NOT NULL AUTO_INCREMENT,`USERNAME` VARCHAR(24) NOT NULL,`Date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,`IP` VARCHAR(16) NOT NULL, PRIMARY KEY (`ID`))");
	mysql_tquery(Database, "CREATE TABLE IF NOT EXISTS `PLAYERDATA` (`DataID` INT NOT NULL AUTO_INCREMENT, `ID` INT NOT NULL, `BANKMONEY` INT NOT NULL DEFAULT '0', `TIME` INT NOT NULL DEFAULT '0', `REGISTERDATE` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `COUNTRY` VARCHAR(36) NOT NULL, `ADMIN` INT NOT NULL DEFAULT '0', `VIP` INT NOT NULL DEFAULT '0', `KILLS` INT NOT NULL DEFAULT '0', `DEATHS` INT DEFAULT '0', `MUTED` INT, `JAILED` INT, PRIMARY KEY (`DataID`))");
	return 1;
}
